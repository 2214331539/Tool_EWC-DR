...
GPU_IDS=${GPU_IDS:-""}
RUN_PREFIX=${RUN_PREFIX:-toolhcl_ewcdr_v2_multiseed}
...
for index in "${!seed_values[@]}"; do
  ...
  selection_dir="${EWC_ARTIFACT_ROOT}/runs/${run_name}_selection"
  log_path="${EWC_ARTIFACT_ROOT}/multiseed_logs/${run_name}.log"

  for path in "${output_dir}" "${selection_dir}" "${EWC_ROOT}/${run_name}"; do
